                ...
                <Button className="bg-transparent hover:bg-transparen text-gray-500 hover:text-black transition-colors outline-none p-0 shadow-none cursor-pointer">
                  <Plus height={20} width={20} onClick={addCard}/>
						...
						<KanbanCards id={column.id}>
							{(task) => (
								<KanbanCard column={column.name} key={task.id} id={task.id} name={task.name} className="cursor-pointer hover:bg-gray-50">
                  <div key={task.id} onPointerDown={(e) => e.stopPropagation()} onClick={() => onCardClick(task)}>

									<div className="">
                    <Input type="text" onClick={(e) => e.stopPropagation()} value={task.name} className=" disabled:opacity-100 disabled:text-black disabled:bg-transparent disabled:border-none disabled:shadow-none border-none bg-transparent shadow-none outline-none" disabled={editableCard == task.id? false : false}/>
									</div>
                  </div>
								</KanbanCard>
							)}
